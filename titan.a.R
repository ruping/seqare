...
runTitan <- function(sampleName, snpFile, tumWig, normWig, gc, map, plp, plpe, normalc, normalcm, symmetric, transtate, tranclone, exons="SRP") {
    ...
    for (j in 1:3) {
        ...
        if (exons == "SRP") {  #WGS
            ...
            par(pty="m")
            par(mar=c(4,4,2,1))
            plotCNlogRByChr(results, ploidy = ploidy, ylim = c(-2, 2), cex=0.25, chr=1:22,
                            main=paste(sampleName, " nc=", numClusters, sep=""),
            ...
            par(mar=c(4,4,2,1))
            plotAllelicRatio(results, ylim = c(0, 1), chr=1:22,
          ...
          par(pty="m")
          par(mar=c(4,4,2,1))
          plotCNlogRByChr(results, ploidy = ploidy, normal=norm, ylim = c(-2, 2), cex=0.25, chr=1:22,
                          main=paste(sampleName, " nc=", numClusters, sep=""),
                          xlab=paste("normC=", round(norm,3), " pl=", ploidy,
                                     " cellularity=", round(cellularity,3),
                                     " SD=",SD," s=",s," nc=",nclones," np=",npoints,"
